...
GOTEMPCOPYPATH?=$(GOTEMPPATH)/src/session-manager-plugin
...
quick-test:
	...
	# go test -gcflags "-N -l" session-manager-plugin/src/datachannel
	go clean -testcache
	go test -cover -gcflags "-N -l" session-manager-plugin/src/...
